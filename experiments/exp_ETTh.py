...
class Exp_ETTh(Exp_Basic):
    ...
    def _get_data(self, flag):
        ...
        def getitem(data_i, index):
            s_begin = index
            s_end = s_begin + args.seq_len
            r_begin = s_end - args.label_len 
            r_end = r_begin + args.label_len + args.pred_len
            x, y, x_mark, y_mark = data_i
            seq_x = x[s_begin:s_end]
            seq_y = y[r_begin:r_end]
            seq_x_mark = x_mark[s_begin:s_end]
            seq_y_mark = y_mark[r_begin:r_end]
            return seq_x, seq_y, seq_x_mark, seq_y_mark

        data_loaders = []
        n_total = 0
        for i in tqdm(range(len(data_set)), desc="-prepare '{0}'".format(flag)):
            data_i = data_set[i]
            n_data = len(data_i[0]) - args.seq_len- args.pred_len + 1
            n_total += n_data
            data_loader = DataLoader(
                [getitem(data_i, row) for row in range(n_data)],
                batch_size=batch_size,
                shuffle=shuffle_flag,
                num_workers=args.num_workers,
                drop_last=drop_last)
            data_loaders.append(data_loader)
        print("Loaded", n_total)
        return data_set, data_loaders
    ...
    def valid(self, valid_data, valid_loaders, criterion):
        self.model.eval()
        total_loss = []
    ...
    def train(self, setting):
        ...
        test_data, test_loaders = self._get_data(flag = 'test')
        ...
        for epoch in range(epoch_start, self.args.train_epochs):
            ...
            print('--------start to test-----------')
            test_loss = self.valid(test_data, test_loaders, criterion)
